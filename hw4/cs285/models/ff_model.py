...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction = ptu.to_numpy(self(obs_unnormalized, acs_unnormalized,
                                        self.obs_mean, self.obs_std, 
                                        self.acs_mean, self.acs_std,
                                        self.delta_mean, self.delta_std)[0])
        return prediction
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        next_obs_unnormalized = ptu.from_numpy(next_observations)
        self.update_statistics(**data_statistics)
        
        target = normalize(next_obs_unnormalized - obs_unnormalized, self.delta_mean, self.delta_std) 
        ...
        prediction = self(obs_unnormalized, acs_unnormalized,
                          self.obs_mean, self.obs_std, 
                          self.acs_mean, self.acs_std,
